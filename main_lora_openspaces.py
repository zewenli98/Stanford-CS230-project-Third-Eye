...
def run_inference_on_test_set(model, processor, num_of_batches=1):
    ...
    for i, batch in enumerate(test_loader):
        ...
        messages = info["messages"]
        reasoning = info["reasoning"]
        for m, r in zip(messages, reasoning):
            output = run_inference(images[0], m, model, processor)
            print("#" * 100)
            print(f"Message: \n{m}")
            print(f"Predicted Reasoning: \n{output}")
...
if __name__ == "__main__":
    ...
    # example: python main_lora_openspaces.py --model=./finetuned_models/LoRA-SpaceThinker-Qwen2.5-VL-3B-Instruct/epoch-1 --processor=./finetuned_models/LoRA-SpaceThinker-Qwen2.5-VL-3B-Instruct/processor --image=./test_img1.jpg --question="I'm blind and holding the camera in my hand. How to reach the cup on the table? Please give a consise and quantitative answer."
